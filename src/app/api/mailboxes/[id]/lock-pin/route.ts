...
export async function PATCH(
  request: NextRequest,
  { params }: RouteContext<"/api/mailboxes/[id]/lock-pin">
) {
  ...
  const { error: rpcError } = await supabase.rpc("set_lock_pin", {
    p_mailbox_id: mailbox.id,
    p_pin: bodyParsed.data.pin,
  });
  if (rpcError) {
    return NextResponse.json({ error: "Failed to update lock PIN" }, { status: 500 });
  }

  return NextResponse.json({ lockPin: bodyParsed.data.pin === null ? null : "set" });
}
